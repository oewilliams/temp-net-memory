...
def main():
	...
	#The pair method is faster here, but both methods included for completeness.
...
def prompt_params(header_check_dict):
	...
	file_loc = input("Enter the location of the edge list file: ")
	seperator = input(r"Enter the seperator character for the edge list (tab is \t without ''): ")
	has_header = input("Does the file have a header line? (True,1,False,0) ")
	...
	min_order = input("Enter minimum order to check: ")
	max_order = input("Enter the maximum order to check: ")
