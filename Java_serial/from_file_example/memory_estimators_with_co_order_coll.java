class memory_estimators_with_co_order_coll {
	
	public memory_estimators_with_co_order_coll() {
		
	}
	
	public static int get_network_pair_memory_est(boolean[][] edge_series, int max_order, int min_order) {
	...
	public static int[][] get_network_memory_shape(boolean[][] edge_series, int max_order, int min_order) {
			...
			int[][] memory_shape = new int[num_links*num_links][3];
	...
	public static int get_network_effective_memory(boolean[][] edge_series, int max_order, int min_order) {
	...
	public static int get_co_order_estimate(int[] target_series, int[] driver_series, int max_order, int min_order, int series_len) {
		...
		for(int i=min_order ; i<=max_order ; i++) {
			double conditional_likelihood = get_kl_dist(target_series, driver_series, series_len, i);
			int num_obs = series_len - 1;
			double EDC_val = -conditional_likelihood + ((1<<(i+1))*(Math.log(Math.log(num_obs))));
			if(EDC_val < EDC_min) {
				EDC_min = EDC_val;
				min_index = i;
			}
